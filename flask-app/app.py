...
def safe_thumbnail_name(image_path):
    # Ensure it matches how thumbnails were saved
    return os.path.basename(image_path).replace(" ", "%20")
        ...
        for entry in index:
            if face_recognition.compare_faces([entry["encoding"]], encoding, tolerance=tolerance)[0]:
                matches.append({
                    ...
                    "thumb_name": safe_thumbnail_name(entry["path"])
                })
    return matches
