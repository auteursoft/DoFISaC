...
CLUSTER_JSON_PHASH = "phash_clusters.json"
CLUSTER_JSON_BG = "bg_clusters.json"
...
def load_vectors():
    ...
    for entry in face_db:
        if entry.get("face_vec") is not None and entry.get("bg_vec") is not None:
...
def main():
    ...
    clustering = DBSCAN(eps=30, min_samples=3).fit(vectors)
    ...
    with open(CLUSTER_JSON_PHASH, "w") as f:
        json.dump(cluster_map, f, indent=2)
    with open(CLUSTER_JSON_BG, "w") as f:
        json.dump(cluster_map, f, indent=2)
